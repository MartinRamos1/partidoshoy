    ...
    <main className="min-h-screen text-slate-100">
            <div className="mx-auto max-w-6xl px-3 sm:px-4 md:px-6 py-6 sm:py-8 md:py-10">
                <div className="flex items-center justify-center gap-4">
                    <button className="text-2xl sm:text-3xl font-semibold mt-2 hover:text-gray-500 cursor-pointer" onClick={() => {
                        const newDate = subtractDayFromDate(matchesDate)
                        console.log(newDate)
                        setMatchesDate(newDate)
                    }}><IoIosArrowBack /></button>
                    <h1 className="text-2xl sm:text-3xl font-semibold">{getDateLabel(matchesDate)}</h1>
                    <button className="text-2xl sm:text-3xl font-semibold mt-2 hover:text-gray-500 cursor-pointer" onClick={() => {
